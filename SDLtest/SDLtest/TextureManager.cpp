...
void TextureManager::Draw(std::string id, int x, int y, int width, int height, float scaleX, float scaleY, float scrollRatio, SDL_RendererFlip flip) {
	...
	Vector2D cam = Camera::GetInstance()->GetPosition() * scrollRatio; // slow the speed of bg by scrollRatio < 1
...
void TextureManager::DrawTile(std::string tilesetID, int tileSize, int x, int y, int row, int col, SDL_RendererFlip flip) {
	...
	Vector2D cam = Camera::GetInstance()->GetPosition();
	SDL_Rect dstRect = { x - cam.X, y - cam.Y, tileSize, tileSize }; // 相当于从屏幕更左边开始画，这样框就显示的中间区域
	SDL_RenderCopyEx(Engine::GetInstance()->GetRenderer(), m_TextureMap[tilesetID], &srcRect, &dstRect, 0, 0, flip);
}

// 把人物和背景图片加载放在一个函数中，信息存在tml文件中,这样每次更新texture就不用重新编译项目
...
void TextureManager::Clean() {
	// after the program closed, we need to destroy everything
